...
const RoomReservation = ({ userData, navigateToHome }) => {
  ...
  const [newRoom, setNewRoom] = useState({ RoomNumber: "", RoomName: "", Capacity: "", Notes: "" });
  ...
  const handleBookRoom = async () => {
    const response = await fetch("/api/bookRoom", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ RoomID: selectedRoom, UserID: userData.UserID, Duration: bookingDuration }),
    });
    const data = await response.json();
    if (data.success) {
  ...
  return (
    <div className="content-container">
      ...
      <div>
        ...
        {rooms.map((room) => (
          <div key={room.RoomID}>
            <p>{room.RoomName} (Capacity: {room.Capacity})</p>
            ...
            {room.isAvailable && userData?.Role !== "Admin" && (
              <div>
                ...
                >
                  ...
                  {userData?.Role === "Faculty" && <option value="3">3 Hours</option>}
                </select>
                <button onClick={() => setSelectedRoom(room.RoomID)}>Book Room</button>
              </div>
            )}
          </div>
        ))}
